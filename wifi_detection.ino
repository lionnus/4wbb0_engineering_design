...
void loop() {
  ...
  if (hour==24&&prevHour<24) {
    ...
    if (WiFi.status()==WL_CONNECTED){
      uploadData();
      outsideTime=0;  //time user spend outside back to 0 mins
    }
    else {
  ...
  if (currConnectionState == WL_CONNECTED) {
      ...
      if (prevConnectionState!=WL_CONNECTED){
          ...
          uploadData();
          Serial.print("Total time outside: "); Serial.println(outsideTime);
  }
        prevTimeConnected=0;
...
void uploadData(){
     Firebase.setString(firebaseData,userString, currDate);
     Firebase.setInt(firebaseData,dataString, outsideTime);
